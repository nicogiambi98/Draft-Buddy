...
# DNS fallback for Android: some environments fail standard libc DNS resolution
# leading to '[Errno 7] No address associated with hostname'. As a minimal
# workaround, we monkey-patch socket.getaddrinfo for our fixed host using
# DNS-over-HTTPS (Cloudflare) resolution when needed. This is only activated
# if the normal resolution fails at runtime.
_DNS_FALLBACK_INSTALLED = False

def _install_dns_fallback_for_host(hostname: str):
    global _DNS_FALLBACK_INSTALLED
    if _DNS_FALLBACK_INSTALLED:
        return
    try:
        import socket
        # First, try a normal resolution; if it works, do nothing.
        try:
        ...
        def _patched_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
            if isinstance(host, str) and host.lower() == hostname.lower():
                res = []
                for ip in ips:
                    res.append((socket.AF_INET, socket.SOCK_STREAM if type in (0, socket.SOCK_STREAM) else type or socket.SOCK_STREAM,
                                socket.IPPROTO_TCP, '', (ip, port)))
                # Fallback to at least one result to avoid empty list
                if not res:
                    res = [(socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_TCP, '', (ips[0], port))]
                return res
...
# Attempt to install DNS fallback on Android at import time
try:
    from kivy.utils import platform as _plat
    if str(_plat) == 'android':
        _install_dns_fallback_for_host('draftbuddy.hackthep.it')
except Exception:
    pass
